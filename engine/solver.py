...
# 权重是业务取舍，不是技术参数 —— 这组默认值对着原始数据调过：
# 原来手工排班 85% 的老师当天只在一个中心，说明运营上很看重这件事。
# 跨中心只值 20 分的话，求解器会为了凑连堂满城调人，学生侧赢一点、老师侧亏一片。
DEFAULT_WEIGHTS = {
    "未排上团队": 100000,
    "连堂不相邻": 60,
    "硬约束倍数": 200,        # 标了「硬」的连堂规则，罚分放大这么多倍
    "跨中心一次": 150,        # 老师当天多跑一个中心的代价
    "每公里转场": 6,
    "课表空档一段": 8,
    "工作量峰值": 30,
    "与原排班不同": 0,        # 修复模式下调高
